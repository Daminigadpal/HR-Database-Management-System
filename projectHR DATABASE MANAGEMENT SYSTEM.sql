...
INSERT INTO 
employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) 
VALUES (177,'Jack','Livingston','jack.livingston@sqltutorial.org',NULL,'1998-04
23',16,8400.00,100,8); 
INSERT INTO 
employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) 
...
) AS src(employee_id, first_name, last_name, email, phone_number, hire_date, job_id, salary, manager_id, department_id)
WHERE src.employee_id NOT IN (SELECT employee_id FROM employees);


SELECT COUNT(*) FROM employees;   -- should now be 40

;WITH expected(employee_id) AS (
    SELECT v FROM (VALUES
    (100),(101),(102),(103),(104),(105),(106),(107),(108),(109),
    (110),(111),(112),(113),(114),(115),(116),(117),(118),(119),
    (120),(121),(122),(123),(126),(145),(146),(176),(177),(178),
    (179),(192),(193),(200),(201),(202),(203),(204),(205),(206)
    ) AS t(v)
)
SELECT e.employee_id AS expected_id
FROM expected e
WHERE e.employee_id NOT IN (SELECT employee_id FROM employees);
...
SELECT COUNT(*) FROM employees;   -- should now be 40
SELECT COUNT(*) FROM dependents;   -- should be 30
...
SELECT e.dependent_id AS expected_id
FROM expected e
WHERE e.dependent_id NOT IN (SELECT dependent_id FROM dependents); 
...
SELECT COUNT(*) FROM dependents;   -- should now be 30
...
WHERE 
condition; 
The WHERE clause appears immediately after the FROM clause. The WHERE clause contains one 
or more logical expressions that evaluate each row in the table. If a row that causes the condition 
evaluates to true, it will be included in the result set; otherwise, it will be excluded. 
Note that SQL has three-valued logic which are TRUE, FALSE, and UNKNOWN. It means that 
if a row causes the condition to evaluate to FALSE or NULL, the row will not be returned. 
Note that the logical expression that follows the WHERE clause is also known as a predicate. You 
can use various operators to form the row selection criteria used in the WHERE clause. 
Operator 
Meaning 
= 
Equal to 
...
selct * from employees where phone_number is null;
D. finds all employees whose salaries are between 9,000 and 12,000. 
 select * from employees where salary between 9,000 and 12000;
...
F. finds all employees whose first name starts with the string jo 
    select * from employees where frist_name like jo%;
G. finds all employees with the first names whose the second character is  h 
 select * from employees where frist_name like _h%;
...
INNER JOIN employees m ON e.manager_id = m.employee_id;
 The president does not have any manager. In the employees table, the manager_id of the row that 
contains the president is NULL.
Because the inner join clause only includes the rows that have matching rows in the other table, 
therefore the president did not show up in the result set of the query above.
SELECT e.first_name + ' ' + e.last_name AS employee,
       m.first_name + ' ' + m.last_name AS manager
FROM employees e
...
SELECT employee_id, first_name, last_name, hire_date,
       CASE
           WHEN YEAR(hire_date) = 2000 THEN 0
           ELSE 2000 - YEAR(hire_date)
